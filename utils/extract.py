...
def main():
    ...
    # Support both: list of dicts, or {"items": [dict, ...]} structure
    items = type_list
    if isinstance(type_list, dict) and "items" in type_list:
        items = type_list["items"]
    ...
    total_nodes = sum(len(v) for p, v in all_links.items() if p != "http")
    per_protocol = {p: len(v) for p, v in all_links.items() if p != "http"}
    ...
    # Compose the report with summary at the top
    report = {"summary": header, "items": report_entries}
