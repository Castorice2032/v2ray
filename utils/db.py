...
# دریافت مقدار یک ستون خاص از یک ردیف خاص (بر اساس id)
def get_column_by_id(table_name, column_name, row_id):
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor()
...
# دریافت ردیف‌ها بر اساس مقدار type (مثلاً فقط سرورهای trojan)
def get_rows_by_type(table_name, type_value, limit=10):
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor()
    query = f'SELECT * FROM "{table_name}" WHERE type = %s ORDER BY created_at DESC LIMIT %s'
    cur.execute(query, (type_value, limit))
    rows = cur.fetchall()
    colnames = [desc[0] for desc in cur.description]
    cur.close()
    conn.close()
...
# افزودن یک ردیف جدید به جدول (داده‌ها به صورت دیکشنری)
def insert_row(table_name, data_dict):
    ...
    query = f'INSERT INTO "{table_name}" ({columns}) VALUES ({values_placeholders}) RETURNING id'
